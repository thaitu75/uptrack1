...
file_handler = RotatingFileHandler('worker.log', maxBytes=5*1024*1024, backupCount=5)
...
def send_telegram_message(message):
    ...
    try:
        ...
        app_logger.error(f"Telegram notification failed: {str(e)}")
...
def process_orders():
    ...
    try:
        while True:
            try:
                ...
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
                                ...
                                while retries < max_retries:
                                    try:
                                        ...
                                        if response.status_code == 429:
                                            ...
                                            app_logger.warning(f"Server error {response.status_code}. Retrying...")
                                            ...
                                            if api_call_limit:
                                                ...
                                                app_logger.debug(f"API Call Limit: {current_calls}/{max_calls}")
                                        ...
                                        app_logger.error(f"Request failed: {str(e)}. Retrying...")
                                        time.sleep(2 ** retries)
                                        retries += 1
                    ...
                    # Clean up old orders
                    clean_up_orders(conn)

            except Exception as e:
                app_logger.error(f"Unexpected error in processing loop: {str(e)}")
        ...
        app_logger.critical(f"Critical error: {str(e)}")
    finally:
        # Close the connection pool
        db_pool.closeall()

def clean_up_orders(conn):
    """Delete orders older than 30 days to prevent the database from becoming full."""
    try:
        with conn.cursor() as cursor:
            ...
            conn.commit()
            if deleted_rows > 0:
                app_logger.info(f"Cleaned up {deleted_rows} old orders from the database.")
    except Exception as e:
        app_logger.error(f"Error during order cleanup: {str(e)}")
